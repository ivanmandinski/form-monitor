    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h1 class="text-2xl font-bold text-gray-900">Edit Form Target</h1>
        ...
        <form action="{{ route('admin.forms.update', $form) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="target_id" id="target_id" required class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach(\App\Models\Target::all() as $target)
                        <option value="{{ $target->id }}" {{ old('target_id', $form->target_id) == $target->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="selector_type" id="selector_type" required class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="css" {{ old('selector_type', $form->selector_type) == 'css' ? 'selected' : '' }}>CSS Selector</option>
                        <option value="xpath" {{ old('selector_type', $form->selector_type) == 'xpath' ? 'selected' : '' }}>XPath</option>
                        <option value="id" {{ old('selector_type', $form->selector_type) == 'id' ? 'selected' : '' }}>Element ID</option>
                        <option value="name" {{ old('selector_type', $form->selector_type) == 'name' ? 'selected' : '' }}>Element Name</option>
                    </select>
                    @error('selector_type')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="selector_value" id="selector_value" required 
                           value="{{ old('selector_value', $form->selector_value) }}"
                ...
                <div class="space-y-3">
                    <div class="flex items-center">
                        <input type="radio" name="driver_type" id="driver_http" value="http" 
                               {{ old('driver_type', $form->driver_type) == 'http' ? 'checked' : '' }}
                    ...
                    <div class="flex items-center">
                        <input type="radio" name="driver_type" id="driver_dusk" value="dusk" 
                               {{ old('driver_type', $form->driver_type) == 'dusk' ? 'checked' : '' }}
                    ...
                    <div class="flex items-center">
                        <input type="radio" name="driver_type" id="driver_puppeteer" value="puppeteer" 
                               {{ old('driver_type', $form->driver_type) == 'puppeteer' ? 'checked' : '' }}
                            ...
                            <div class="mt-3 space-y-2">
                                <label for="recaptcha_expected" class="flex items-center">
                                    <input type="checkbox" name="recaptcha_expected" id="recaptcha_expected" value="1" 
                                           {{ old('recaptcha_expected', $form->recaptcha_expected) ? 'checked' : '' }}
                                ...
                                <label for="uses_js" class="flex items-center">
                                    <input type="checkbox" name="uses_js" id="uses_js" value="1" 
                                           {{ old('uses_js', $form->uses_js) ? 'checked' : '' }}
                                           class="h-4 w-4 text-green-600 focus:ring-green-500 border-green-300 rounded">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="success_selector" id="success_selector" 
                           value="{{ old('success_selector', $form->success_selector) }}"
                           class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                    ...
                    <input type="text" name="error_selector" id="error_selector" 
                           value="{{ old('error_selector', $form->error_selector) }}"
                           class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div>
                        <label for="schedule_enabled" class="flex items-center">
                            <input type="checkbox" name="schedule_enabled" id="schedule_enabled" value="1" 
                                   {{ old('schedule_enabled', $form->schedule_enabled) ? 'checked' : '' }}
                    ...
                    <div id="schedule-options" class="grid grid-cols-1 lg:grid-cols-2 gap-6" style="display: none;">
                        <div>
                            ...
                            <select name="schedule_frequency" id="schedule_frequency" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="hourly" {{ old('schedule_frequency', $form->schedule_frequency) == 'hourly' ? 'selected' : '' }}>Hourly</option>
                                <option value="daily" {{ old('schedule_frequency', $form->schedule_frequency) == 'daily' ? 'selected' : '' }}>Daily</option>
                                <option value="weekly" {{ old('schedule_frequency', $form->schedule_frequency) == 'weekly' ? 'selected' : '' }}>Weekly</option>
                                <option value="cron" {{ old('schedule_frequency', $form->schedule_frequency) == 'cron' ? 'selected' : '' }}>Custom Cron</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="schedule_timezone" id="schedule_timezone" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                <option value="UTC" {{ old('schedule_timezone', $form->schedule_timezone) == 'UTC' ? 'selected' : '' }}>UTC</option>
                                <option value="America/New_York" {{ old('schedule_timezone', $form->schedule_timezone) == 'America/New_York' ? 'selected' : '' }}>Eastern Time</option>
                                <option value="America/Chicago" {{ old('schedule_timezone', $form->schedule_timezone) == 'America/Chicago' ? 'selected' : '' }}>Central Time</option>
                                <option value="America/Denver" {{ old('schedule_timezone', $form->schedule_timezone) == 'America/Denver' ? 'selected' : '' }}>Mountain Time</option>
                                <option value="America/Los_Angeles" {{ old('schedule_timezone', $form->schedule_timezone) == 'America/Los_Angeles' ? 'selected' : '' }}>Pacific Time</option>
                                <option value="Europe/London" {{ old('schedule_timezone', $form->schedule_timezone) == 'Europe/London' ? 'selected' : '' }}>London</option>
                                <option value="Europe/Paris" {{ old('schedule_timezone', $form->schedule_timezone) == 'Europe/Paris' ? 'selected' : '' }}>Paris</option>
                                <option value="Asia/Tokyo" {{ old('schedule_timezone', $form->schedule_timezone) == 'Asia/Tokyo' ? 'selected' : '' }}>Tokyo</option>
                            </select>
                        </div>
                        ...
                        <div id="cron-input" style="display: none;">
                            ...
                            <input type="text" name="schedule_cron" id="schedule_cron" 
                                   value="{{ old('schedule_cron', $form->schedule_cron) }}"
                                   class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
